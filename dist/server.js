...
app.get("/oauth/etsy/callback", async (req, res) => {
    ...
    if (row.openai_state)
        redirect.searchParams.set("state", row.openai_state);
...
function createMcpServer(session) {
    ...
    server.tool("etsy_get_shop", "Get Etsy shop information.", { shop_id: z.number().int().positive() }, async ({ shop_id }) => json(await etsy(session, "GET", `/shops/${shop_id}`)));
    server.tool("etsy_get_listings", "Get listings for a shop.", {
    ...
    server.tool("etsy_create_listing", "Create a draft Etsy listing.", {
        shop_id: z.number().int().positive(),
    ...
    server.tool("etsy_update_listing", "Update an Etsy listing.", {
        shop_id: z.number().int().positive(),
    ...
    server.tool("etsy_get_orders", "Get shop receipts/orders.", {
        shop_id: z.number().int().positive(),
        limit: z.number().int().min(1).max(100).optional(),
        offset: z.number().int().min(0).optional(),
    ...
    server.tool("etsy_get_messages", "Not supported by Etsy Open API v3. Etsy's public OpenAPI spec does not expose conversations/messages endpoints.", { shop_id: z.number().int().positive() }, async () => unsupportedMessages());
    server.tool("etsy_reply_to_message", "Not supported by Etsy Open API v3. Etsy's public OpenAPI spec does not expose reply-to-message endpoints.", {
        shop_id: z.number().int().positive(),
...
async function etsy(session, method, path, query, body) {
...
async function validEtsyAccessToken(session) {
    if (session.etsy_expires_at > Date.now())
        return session.etsy_access_token;
...
async function exchangeEtsyCode(code, verifier) {
...
async function etsyTokenRequest(params) {
    ...
    if (!response.ok)
        throw new Error(`Etsy token request failed: ${JSON.stringify(data)}`);
    return data;
}
...
class JsonStore {
    ...
    updateSessionByCode(code, patch) {
    ...
    updateSessionByAccessToken(token, patch) {
        if (!token)
            return;
